...
from environment.games import TicTacToe, alpha_beta_player, random_player, Fig52Extended
from environment.learning import DataSet
from environment.logic import parse_definite_clause, standardize_variables, unify_mm, subst
from environment.search import GraphProblem, romania_map
...
def _psource_style():
    """Build CSS for psource that is scoped to .psource and follows the notebook theme."""
    from pygments.formatters import HtmlFormatter

    def scoped(style_name):
        css = HtmlFormatter(style=style_name).get_style_defs('.psource')
        # Keep only the token rules. Pygments also emits unscoped rules for
        # `pre` and `td.linenos`, plus a `.psource { background: ... }` rule;
        # those are what turn the whole notebook page white.
        return '\n'.join(line for line in css.splitlines() if line.startswith('.psource .'))

    try:
        dark = scoped('github-dark')
    except Exception:
        dark = scoped('monokai')
...
def psource(*functions):
    ...
    try:
        ...
        body = highlight(source_code, PythonLexer(), HtmlFormatter(cssclass='psource'))
        display(HTML(_psource_style() + body))
